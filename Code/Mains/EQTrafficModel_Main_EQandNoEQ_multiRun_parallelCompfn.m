...
vmaxMat=zeros(noEn,RG.numLinks);
...
for i=1:noEn
    ...
    if isImperfParams==1
        ...
        [vmaxVec,nvalueLinks]=alterParams(RG.valueLinks);
        mapLinksApp_wEQ_array{i}=containers.Map(RG.keyLinks,nvalueLinks); % filter, EQ
        mapLinksApp2_wEQ_array{i}=containers.Map(RG.keyLinks,nvalueLinks); % open loop, EQ
        mapLinksApp_nEQ_array{i}=containers.Map(RG.keyLinks,nvalueLinks); % filter, no EQ
        mapLinksApp2_nEQ_array{i}=containers.Map(RG.keyLinks,nvalueLinks); % open loop, no EQ
        ...
        mapLinksApp_wEQ_array{i}=containers.Map(RG.keyLinks,RG.valueLinks); % filter, EQ
        mapLinksApp2_wEQ_array{i}=containers.Map(RG.keyLinks,RG.valueLinks); % open loop, EQ
        mapLinksApp_nEQ_array{i}=containers.Map(RG.keyLinks,RG.valueLinks); % filter, no EQ
        mapLinksApp2_nEQ_array{i}=containers.Map(RG.keyLinks,RG.valueLinks); % open loop, no EQ
...
maxvmaxVec=zeros(1,RG.numLinks);
...
% mapBridges
mapBridges=containers.Map(RG.keyBridges,RG.valueBridges);
...
mapLinksNoise=getCellDiscImperf(DT,mapLinksNoise,RG.keyLinks,RG.numLinks,maxvmaxVec);
...
for i=1:noEn
    ...
    mapLinksApp_wEQ=getCellDiscImperf(DT,mapLinksApp_wEQ,RG.keyLinks,RG.numLinks,maxvmaxVec);
    mapLinksApp2_wEQ=getCellDiscImperf(DT,mapLinksApp2_wEQ,RG.keyLinks,RG.numLinks,maxvmaxVec);
    mapLinksApp_nEQ=getCellDiscImperf(DT,mapLinksApp_nEQ,RG.keyLinks,RG.numLinks,maxvmaxVec);
    mapLinksApp2_nEQ=getCellDiscImperf(DT,mapLinksApp2_nEQ,RG.keyLinks,RG.numLinks,maxvmaxVec);
...
mapLinksNoise=getBridges(mapLinksNoise,RG.numLinks,mapBridges,RG.numBridges);
...
for i=1:noEn
    ...
    mapLinksApp_wEQ=getBridges(mapLinksApp_wEQ,RG.numLinks,mapBridges,RG.numBridges);
    mapLinksApp2_wEQ=getBridges(mapLinksApp2_wEQ,RG.numLinks,mapBridges,RG.numBridges);
    mapLinksApp_nEQ=getBridges(mapLinksApp_nEQ,RG.numLinks,mapBridges,RG.numBridges);
    mapLinksApp2_nEQ=getBridges(mapLinksApp2_nEQ,RG.numLinks,mapBridges,RG.numBridges);
...
[Heq sensInd]=getHeq(mapSensors,RG.keySensors,totCells,RG.numSensors,pf_sens);
...
mapLinksNoise=getDistanceEQ(mapLinksNoise,RG.numLinks,mapBridges);

% Construct vector of distances
distVecN=getDistVec(mapLinksNoise,RG.keyLinks,RG.numBridges,RG.numLinks);

% Get peak ground accelerations
PGAN=getPGA(RG.newEQ.mag,distVecN,F,mapLinksNoise,RG.keyLinks,mapBridges);

% Create deterministic fragility curves for fragility model. 
p=getFragCurves(PGAVec,mapLinksNoise,RG.keyLinks,mapBridges,RG.numBridges);
...
for k=1:length(measTime)-1
    ...
    if k==keq
        ...
        mapLinksNoise=getMapParamsN(mapLinksNoise,RG.numLinks,RG.keyLinks,mapBridges);
...
for k=1:K-1
    ...
    for i=1:noEn
        ...
        if k>=keq
            ...
            m_wEQ=getEQParams(1,RG.newEQ);
            m_nEQ=getEQParams(0,RG.newEQ);
            ...
            % mapLink objects.
            mapLinksApp_wEQ=getDistanceEQDist(mapLinksApp_wEQ,RG.numLinks,mapBridges,1);
            mapLinksApp2_wEQ=getDistanceEQDist(mapLinksApp2_wEQ,RG.numLinks,mapBridges,1);
            
            mapLinksApp_nEQ=getDistanceEQDist(mapLinksApp_nEQ,RG.numLinks,mapBridges,0);
            mapLinksApp2_nEQ=getDistanceEQDist(mapLinksApp2_nEQ,RG.numLinks,mapBridges,0);
            
            % Construct vector of distances, R. Only need one set of
            % params for wEQ and one set of params for nEQ. Open loop or
            % filter does not matter.
            distVec_wEQ=getDistVec(mapLinksApp_wEQ,RG.keyLinks,RG.numBridges,RG.numLinks);
            distVec_nEQ=getDistVec(mapLinksApp_nEQ,RG.keyLinks,RG.numBridges,RG.numLinks);
            
            % Get spectral accelerations
            PGA_wEQ=getPGA(nEQ_wEQ.mag,distVec_wEQ,F,mapLinksApp_wEQ,RG.keyLinks,mapBridges);
            PGA_nEQ=getPGA(nEQ_nEQ.mag,distVec_nEQ,F,mapLinksApp_nEQ,RG.keyLinks,mapBridges);
    ...
    if k>=keq
        ...
        for i=1:noEn
            ...
            [ensMeas ensMeasNoise]=getEnsMeasNoiseEQ(meas,mapSensors,RG.keySensors,sensInd);
